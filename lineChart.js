function linePlot(id, args){
    ...
    var dateFn = function(d){ return timeFormat.parse(d) };
    ...
    this.plot = function(data, args){
        ...
        if( data_all.length !== 0 ){
            minBound = Math.min(d3.min(data_all, function(d) { return d.y; }) - extraY, bound.min)
            maxBound = Math.max(d3.max(data_all, function(d) { return d.y; }) + extraY, bound.max)
        ...
        if(args.inHospital !== "undefined"){
            for(var key in args.inHospital){
                ...
                svg.select(".inHospital")
                    ...
                    .append('title')
                    .text(function(d) {
                        return args.inHospital[key].cause + '\n' + t1.replace("T", " ") + '\n' + t2.replace("T", " ");
                    });
                if(args.inHospital[key].cause !== "undefined" && args.timeAxisFormat !== "month"){
                    svg.select(".inHospital")
                        .append('text')
                        .text(args.inHospital[key].cause)
                        .attr({'x': xScale(timeFormat.parse(t1)),
                                'y': this.padding*0.9,
                                'class': 'cause',
        ...
        for( var i=0; i < data.length; i++){
            ...
            svg.selectAll('.dataPoint'+i)
                .data(data[i])
                .enter()
                .append('circle')
                .attr({
                    'cx': function(d){ return xScale(dateFn(d.x)); },
                    ...
                    if( (d.y > args.normalRange[i].upper) || (d.y < args.normalRange[i].lower)){
                        d3.select(this)
                            .attr({'stroke': args.dataPointStrokeAlert[i],
                                    'stroke-width': args.dataLineStrokeWidth[i],
    ...
    this.update = function(data, args){
        ...
        svg.selectAll(".inHospital").selectAll('rect').remove();
        ...
        for( var i=0; i < data.length; i++){
            ...
            var circles = svg.selectAll('.dataPoint'+i)
                                .data(data[i]);
            ...
            svg.select('.upperLine'+i)
                .transition()
                .duration(this.duration)
                .attr('d', line(upper));
            
            svg.select('.lowerLine'+i)
                .transition()
                .duration(this.duration)
                .attr('d', line(lower));
        ...
        text.each(function(){
            ...
            switch(timeAxisFormat){
                case "%m/%d %a":
                    ...
                    break;
                case "%m/%d %H:%M":
                    tspan = text.append("tspan").attr("x", 0).attr("y", y).attr("dy", ++lineNumber * lineHeight + dy + "em").text(words.pop());
                    break;
            }
